	...
	with(argument0){
		if(show_inventory){
			...
			repeat(inv_slots){
				...
				//item
				iitem = global.ds_inventory[# 0, ii];
				...
				//draw number
				if(iitem > 0){
					var number = global.ds_inventory[# 1, ii];
...
else if(argument0 == obj_actionbar){
	with(obj_actionbar){
			//actionbar
